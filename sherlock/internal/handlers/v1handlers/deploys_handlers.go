...
import (
	...
	"github.com/broadinstitute/sherlock/sherlock/internal/metrics/v1metrics"
...
func createDeploy(dc *v1controllers.DeployController) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		v1metrics.RecordDeployFrequency(c, environment, service)
		// calculate lead time
		if shouldUpdateLeadTime {
			leadTime := deploy.CalculateLeadTimeHours()
			v1metrics.RecordLeadTime(c, leadTime, environment, service)
		}
