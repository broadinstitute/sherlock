...
func Test_propagatorImpl_consumeStatesToDiff(t *testing.T) {
	...
	alignmentOperations := p.consumeStatesToDiff(ctx, grant, currentState, desiredState)
...
func Test_propagatorImpl_consumeStatesToDiff_empty(t *testing.T) {
	engine := propagation_engines_mocks.NewMockPropagationEngine[string, testIdentifier, testFields](t)
	p := &propagatorImpl[string, testIdentifier, testFields]{
		engine: engine,
	}
