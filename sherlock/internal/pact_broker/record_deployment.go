...
func UseMockedPactBroker(t *testing.T, config func(c *pact_broker_mocks.MockMockablePactBroker), callback func()) {
	if config == nil {
		callback()
		return
	}
	c := pact_broker_mocks.NewMockMockablePactBroker(t)
	config(c)
	temp := pactbroker
	pactbroker = c
	callback()
