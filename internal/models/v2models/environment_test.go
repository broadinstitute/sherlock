...
func Test_validateEnvironment(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "invalid static with offline schedule true",
			args: args{environment: &Environment{
				Name:                       "foobar",
				Lifecycle:                  "static",
				UniqueResourcePrefix:       "a1b2",
				...
				DefaultFirecloudDevelopRef: testutils.PointerTo("dev"),
				Offline:                    testutils.PointerTo(false),
				OfflineScheduleEnabled:     testutils.PointerTo(true),
			}},
			wantErr: true,
		},
		{
			name: "invalid dynamic with offline schedule true",
			args: args{environment: &Environment{
				Name:                       "foobar",
				Lifecycle:                  "dynamic",
				UniqueResourcePrefix:       "a1b2",
		...
		},
		{
			...
			args: args{environment: &Environment{
				...
				Offline:                    testutils.PointerTo(false),
				OfflineScheduleEnabled:     testutils.PointerTo(true),
				OfflineScheduleBegin:       testutils.PointerTo(time.Now()),
				OfflineScheduleEnd:         testutils.PointerTo(time.Now()),
			}},
			wantErr: false,
		},
	}
